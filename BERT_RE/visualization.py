...
def enhanced_plot_metrics_curves(metrics_history, save_dir, fold=None, filename_prefix=""):
    """
    Enhanced version of plot_metrics_curves that handles low-value metrics better.
    ...
    """
    # Check if metrics history is empty or has empty lists
    if not metrics_history or all(len(v) == 0 for v in metrics_history.values()):
        logger.warning("No metrics data to plot. Creating placeholder plot.")
        # Create a placeholder plot with a message
        plt.figure(figsize=(12, 8))
        plt.text(0.5, 0.5, "No metrics data available.\nCheck if validation dataset exists.",
    ...
    plt.savefig(filename, bbox_inches='tight', dpi=150)
    plt.close()
    logger.info(f"Enhanced metrics curves plot saved to {filename}")
